...
public class CursorPosition
{
    ...
    public static CursorResult calculateRaw(Player player, int cursorDistance)
    {
        ...
        Block targetBlock = player.getTargetBlock(null, cursorDistance);
        ...
        for (double d = start; d < cursorDistance; d += /*ACCURACY*/0.0125)
        {
            ...
            if (vectorOnBlock.toBukkitLocation(location.getWorld()).getBlock().getType() != Material.AIR)
            {// Block is solid -> we've hit the target block
    ...
    {
        if (targetVector == null || mapMenu == null)
        {
            return null;
        }
